...
def validate_score_artifact(
    ...
    required = {
        "model", "participant", "condition", "trial_index",
        "choice_log_score", "brier_score", "choice_correct",
    }
    ...
    if set(observed.index) != set(expected_index):
        raise RuntimeError("Held-out score artifact has an incomplete participant/model matrix.")
    ...
    source_masks = run_b.loc[
        :, ["participant", "trial_index", "condition", "choice_included", "rt_included"]
    ].copy()
    ...
    if checked[["choice_included", "rt_included", "condition_source"]].isna().any().any():
        raise RuntimeError("Held-out score keys do not map completely to Run-B trials.")
    if not checked["condition"].astype(str).eq(checked["condition_source"].astype(str)).all():
        raise RuntimeError("Held-out score conditions disagree with Run-B trials.")
    choice_finite = np.isfinite(
        pd.to_numeric(checked["choice_log_score"], errors="coerce").to_numpy(float)
    )
    if not np.array_equal(choice_finite, checked["choice_included"].to_numpy(bool)):
        raise RuntimeError("Choice-score missingness disagrees with the choice mask.")
    full = scores["model"].astype(str).ne("choice_only")
    required_full = {"rt_log_score", "absolute_log_rt_error",
                     "absolute_rt_error_seconds", "out_of_support"}
    ...
    full_checked = checked.loc[checked["model"].astype(str).ne("choice_only")]
    rt_finite = np.isfinite(
        pd.to_numeric(full_checked["rt_log_score"], errors="coerce").to_numpy(float)
    )
    if not np.array_equal(rt_finite, full_checked["rt_included"].to_numpy(bool)):
        raise RuntimeError("RT-score missingness disagrees with the RT mask.")
    baseline = scores["model"].astype(str).eq("choice_only")
    if scores.loc[baseline, ["rt_log_score", "absolute_log_rt_error",
                             "absolute_rt_error_seconds"]].notna().any().any():
...
def validate_reliability_matrix(
    ...
    observed = [(str(result.participant), str(result.model)) for result in results]
    if set(observed) != expected or len(observed) != len(expected):
        raise RuntimeError("Run-B reliability fits do not form the exact participant/model matrix.")
...
def main() -> None:
    ...
    run_a_starts_path = repository_path(config, "results/run_a_optimizer_starts.csv")
    run_a_runtime_path = repository_path(config, "results/run_a_runtime.json")
    ...
    failed_audit_passes = [
        name for name in required_audit_passes if audit.get(name) is not True
    ]
    ...
    if not run_a_receipt_path.is_file():
        raise RuntimeError("Formal run B requires a completed Run-A receipt.")
    run_a_receipt_hash = sha256_file(run_a_receipt_path)
    run_a_receipt = json.loads(run_a_receipt_path.read_text(encoding="utf-8"))
    ...
    receipt_mismatches = {
        "status": (run_a_receipt.get("status"), "completed"),
        "failures": (run_a_receipt.get("failures"), 0),
        "git_commit": (run_a_receipt.get("git_commit"), commit),
        "config_sha256": (run_a_receipt.get("config_sha256"), config_hash),
        "raw_data_sha256": (run_a_receipt.get("raw_data_sha256"), raw_hash),
        "raw_archive_sha256": (
        ...
        "artifacts": (run_a_receipt.get("artifacts"), run_a_artifact_hashes),
    }
    receipt_mismatches = {
        name: values for name, values in receipt_mismatches.items() if values[0] != values[1]
    }
    if receipt_mismatches:
        raise RuntimeError(
            f"Run-A completion receipt does not match current artifacts: {receipt_mismatches}"
        )
    fingerprint = {
    ...
    if set(run_a_fits["data_pipeline_sha256"].astype(str)) != {data_pipeline_hash}:
        raise RuntimeError("Run-A fits were generated by a different data pipeline.")
    ...
    try:
        full_fits, baseline_fits = validate_run_a_fit_artifact(
            run_a_fits, participants, config
        )
    except ValueError as exc:
        raise RuntimeError(f"Invalid Run-A fit artifact: {exc}") from exc

    if sha256_file(processed_path) != processed_hash:
        raise RuntimeError("Processed data changed while formal preflight loaded it.")
    if sha256_file(run_a_fits_path) != run_a_artifact_hashes["run_a_fits.csv"]:
        raise RuntimeError("Run-A fits changed while formal preflight loaded them.")
    final_preflight = {
        "git_commit": (clean_git_commit(root), commit),
        "config_sha256": (sha256_file(config["_config_path"]), config_hash),
        "audit_sha256": (sha256_file(audit_path), audit_hash),
        "run_a_receipt_sha256": (
            sha256_file(run_a_receipt_path), run_a_receipt_hash
        ),
        "data_pipeline_sha256": (data_pipeline_source_hash(root), data_pipeline_hash),
    }
    changed_preflight = {
        name: pair for name, pair in final_preflight.items() if pair[0] != pair[1]
    }
    if changed_preflight:
        raise RuntimeError(f"Formal inputs changed during preflight: {changed_preflight}")
    ...
    # All deterministic read-only preflight checks are complete. Atomic local
    # lock acquisition now selects one process; only then is the tracked
    # one-shot registry changed and model scoring allowed to begin.
    ...
    atomic_write_json(lock_directory / "reservation.json", reservation)
    ...
    score_frame = pd.concat(score_rows, ignore_index=True, sort=False)
    validate_score_artifact(score_frame, run_b, participants)

    with tempfile.TemporaryDirectory(
        prefix=".formal_run_b_staging_", dir=output_parent
    ) as staging_name:
        staging = Path(staging_name)
        score_frame.to_csv(staging / "trial_scores.csv", index=False)
        ...
        reliability_frame, reliability_failure_count = validate_reliability_matrix(
            reliability_results, participants, config
        )
        failed_reliability = [
    ...
    completed = {
        ...
        "run_b_reliability_failure_count": reliability_failure_count,
    }
    append_manifest(
    ...
    atomic_write_json(lock_directory / "reservation.json", completed)
