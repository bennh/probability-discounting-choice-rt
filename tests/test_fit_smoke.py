...
@unittest.skipUnless(
    SCIPY_AVAILABLE,
    "SciPy is unavailable in the current validation runtime",
)
class FitSmokeTests(unittest.TestCase):
    ...
    def test_penalized_starts_return_explicit_failed_fit(self):
        ...
        self.assertTrue(
            all(np.isnan(value) for value in result.estimates.values())
        )

    def test_map_penalty(self):
        from pd_project.fit import _map_penalty
        ...
        config = {
            "map_fallback": {
                "weak_priors": {
                    ...
                    "log_b": {
                        "family": "normal",
                        "scale_rule": "run_a_predictor_g95",
                        "sd": 1.5,
                        "resolved_g95": {
                            "M1": 11.362737,
                            "M2": 129.352219,
        ...
        parameters = {
            ...
            "alpha": 0.693147,
            "delta_L": 0.0,
            "log_b": -np.log(11.362737),
            "log_sigma": -0.693147,
        }

        penalty = _map_penalty(
            parameters,
            "M1",
            config,
            choice_only=False,
        ...
        self.assertAlmostEqual(penalty, 0.0, places=12)
